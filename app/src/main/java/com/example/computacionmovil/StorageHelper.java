...
public class StorageHelper {
    ...
    public static Recorrido readRecorridoFromFile(String filename, Context context) throws IOException {
        ...
        ArrayList<LatLng> arrayAntenas = new ArrayList<>();
        ...
        while(m.find()) {
            ...
            Pattern p2 = Pattern.compile("\"antena\":((\\d+)(\\.\\d+)?),\"dbm\":((-\\d+)(\\.\\d+)?),\"etapa\":((\\d+)(\\.\\d+)?),\"latitud\":(-?\\d+(\\.\\d+)?),\"longitud\":(-?\\d+(\\.\\d+)?),\"posAntena\":(-?\\d+(\\.\\d+)?)");
            Matcher m2 = p2.matcher(Objects.requireNonNull(m.group(16)));
            int countActualMedida = 0;
            while (m2.find()) {
                arrayMedidasEtapa[countActualMedida] = new Medida(context.getApplicationContext(), Integer.parseInt(Objects.requireNonNull(m2.group(8))), Double.parseDouble(Objects.requireNonNull(m2.group(12))), Double.parseDouble(Objects.requireNonNull(m2.group(10))), Integer.parseInt(Objects.requireNonNull(m2.group(2))), Integer.parseInt(Objects.requireNonNull(m2.group(5))), Integer.parseInt(Objects.requireNonNull(m2.group(14))));
                countActualMedida++;
            }
        ...
        Pattern recorridoPattern = Pattern.compile("\"recorrido\":\\{\"maxMedida\":((-?\\d+)(\\.\\d+)?),\"medMedida\":((-?\\d+)(\\.\\d+)?),\"minMedida\":((-?\\d+)(\\.\\d+)?),\"nMedidas\":(\\d+),\"antenas\":\\[([^\\]]*)?(.*)");
        Matcher recorridoMatcher = recorridoPattern.matcher(content);
        if(recorridoMatcher.find()){
            Pattern antenasPattern = Pattern.compile("\"((-?\\d+)(\\.\\d+)?),((-?\\d+)(\\.\\d+)?)\"");
            Matcher antenasMatcher = antenasPattern.matcher(Objects.requireNonNull(recorridoMatcher.group(11)));
            while(antenasMatcher.find()){
                arrayAntenas.add(new LatLng(Double.parseDouble(Objects.requireNonNull(antenasMatcher.group(1))),Double.parseDouble(Objects.requireNonNull(antenasMatcher.group(4)))));
            }
            r = new Recorrido(context,Integer.parseInt(Objects.requireNonNull(recorridoMatcher.group(7))),Integer.parseInt(Objects.requireNonNull(recorridoMatcher.group(4))),Integer.parseInt(Objects.requireNonNull(recorridoMatcher.group(1))),Integer.parseInt(Objects.requireNonNull(recorridoMatcher.group(10))),arrayAntenas,arrayEtapas);
        }
